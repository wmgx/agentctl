...
- {{threshold}}+轮才能完成：实现功能模块、搭建项目、复杂调试、持续迭代且无对应 skill、用户描述模糊需要多次确认、深度调研对比分析、技术选型评估

意图类型：
- "direct": 预期 1-{{threshold_minus1}} 轮可解决。包括：问答、解释、翻译、写小函数、修单个bug、小段代码生成
- "session": 满足以下任一条件即判断为 session：
  * 预期 {{threshold}}+ 轮才能完成
  * 需要持续迭代、访问本地文件/执行命令/访问外部URL进行深度分析
  * **用户明确指定模型**（如"用opus模型"、"用haiku"、"用claude-3-5-sonnet"等）—— 因为意图分类器使用固定模型，用户指定模型说明要在 session 中使用该模型执行任务
  * 任务类型包括：实现功能模块、搭建项目、深度调研、对比分析（如"调研A和B哪个更适合"）、技术选型、复杂调试
  * 用户明确说"建群/开项目/新建会话"
